...
contract GymMembership {
    ...
    event TreasuryWalletUpdated(address indexed treasuryWallet, uint256 timestamp);
     ...
     */
    function registerMember(string memory _name, uint8 _type) external payable reentrancyGuard {
        ...
        bool existingMember = member.memberAddress != address(0);
        ...
        if (!existingMember) {
            ...
            emit MembershipRenewed(msg.sender, membershipType, expiryDate);
        }
    ...
    function updateTreasuryWallet(address _newTreasuryWallet) external onlyOwner {
        require(_newTreasuryWallet != address(0), "Invalid treasury address");
        treasuryWallet = _newTreasuryWallet;
        emit TreasuryWalletUpdated(_newTreasuryWallet, block.timestamp);
    }
    ...
    function _forwardRevenue(uint256 _amount) internal {
