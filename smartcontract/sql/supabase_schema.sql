...
create table if not exists public.gym_payment_transactions (
  ...
  status text not null default 'pending' check (status in ('pending', 'confirmed', 'failed', 'refunded', 'submitted', 'reverted')),
  block_number bigint,
  paid_at timestamptz,
  metadata jsonb not null default '{}'::jsonb,
  created_at timestamptz not null default now(),
...
create table if not exists public.gym_refunds (
  id bigserial primary key,
  payment_transaction_id bigint references public.gym_payment_transactions(id) on delete cascade,
  original_tx_id text,
  user_id uuid not null references public.app_users(id) on delete cascade,
  amount_wei text,
  refund_amount_wei text,
  tx_hash text unique,
  refund_tx_hash text unique,
  status text not null default 'pending' check (status in ('pending', 'approved', 'paid', 'rejected', 'failed', 'confirmed')),
  reason text,
  metadata jsonb not null default '{}'::jsonb,
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now()
...
alter table public.gym_payment_transactions
  add column if not exists transaction_type text;
alter table public.gym_payment_transactions
  add column if not exists currency text not null default 'ETH';
alter table public.gym_payment_transactions
  add column if not exists blockchain_timestamp bigint;
alter table public.gym_payment_transactions
  add column if not exists payment_kind text;

update public.gym_payment_transactions
set transaction_type = coalesce(transaction_type, payment_kind, 'payment_received')
where transaction_type is null;

alter table public.gym_payment_transactions
  alter column transaction_type set not null;

alter table public.gym_refunds
  add column if not exists original_tx_id text;
alter table public.gym_refunds
  add column if not exists refund_amount_wei text;
alter table public.gym_refunds
  add column if not exists refund_tx_hash text;

update public.gym_refunds
set refund_amount_wei = coalesce(refund_amount_wei, amount_wei)
where refund_amount_wei is null;
...
create index if not exists idx_gym_payment_transactions_type on public.gym_payment_transactions (transaction_type);
create index if not exists idx_gym_refunds_user on public.gym_refunds (user_id);
create index if not exists idx_gym_refunds_refund_tx_hash on public.gym_refunds (refund_tx_hash);
